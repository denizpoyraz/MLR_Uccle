...
jan = [[0]*50]*36 ; feb = [[0]*50]*36; mar = [[0]*50]*36; apr = [[0]*50]*36; may = [[0]*50]*36; jun = [[0]*50]*36;
jul = [[0]*50]*36;  aug = [[0]*50]*36; sep = [[0]*50]*36; oct = [[0]*50]*36; nov = [[0]*50]*36; dec = [[0]*50]*36

jan_mean = [0]*36; feb_mean = [0]*36; mar_mean = [0]*36; apr_mean = [0]*36; may_mean = [0]*36; jun_mean = [0]*36;
jul_mean = [0]*36; aug_mean = [0]*36; sep_mean = [0]*36; oct_mean = [0]*36; nov_mean = [0]*36; dec_mean = [[0]*50]*36


for ir in range(2): # per each km
    alt[ir] = str(ir) + 'km'
    uc[ir] = uccle[uccle[alt[ir]] > 0]
    # or uct[ir] = uc[ir].loc['1987-02-01':'2017-06-01']
    uct[ir] = uc[ir]

    jan[ir].clear(); feb[ir].clear(); mar[ir].clear(); apr[ir].clear(); may[ir].clear(); jun[ir].clear
    jul[ir].clear(); aug[ir].clear(); sep[ir].clear(); oct[ir].clear(); nov[ir].clear(); dec[ir].clear
...
for ir2 in range(26): # per each km
    alt[ir2] = str(ir2) + 'km'
    ...
    #
        if (pd.Timestamp(i2).month == 1):
            #print('before',alt[ir2],  i2, (uct2[ir2][alt[ir2]].loc[pd.Timestamp(i2)]))

            uct2[ir2][alt[ir2]].loc[pd.Timestamp(i2)]  = uct2[ir2][alt[ir2]].loc[pd.Timestamp(i2)]  - jan_mean[ir2]
            #print('after',alt[ir2],  i2, (uct2[ir2][alt[ir2]].loc[pd.Timestamp(i2)]))


#     frames.append(uct[ir2 ])
#     dfde = pd.concat(frames)
#
#
#
#
#
# dfde.to_csv('/home/poyraden/MLR_Uccle/1km_monthlymean_deseas.csv')


make the new data frame!!!!

# utest = uccle[uccle['0km'] > 0]
#
# for ij in(utest['0km'].index):
#     if (pd.Timestamp(ij).month == 1):
#         print('before', ij, (utest['0km'].loc[pd.Timestamp(ij)]))
#         utest['0km'] = utest['0km'].loc[pd.Timestamp(ij)] - jan_mean[0]
#         print('after', ij, (utest['0km'].loc[pd.Timestamp(ij)]))
