...
predictors = predictors.resample('12M',on='dateindex_two').mean()
predictors['dateindex'] = pd.to_datetime(predictors.index, format='%Y-%m')
predictors['dindex'] = predictors.dateindex.dt.date.apply(lambda x: x.strftime('%Y-%m'))
l2 = predictors.dindex.tolist()
...
uccle = uccle[(uccle.Date < '2018-12') & (uccle.Date >= '1972-01-01')]
...
# l1 = uccle.index.tolist() # for sommthed data


uccle = uccle.resample('12M', on='dateindex').mean()
uccle['dateindex'] = pd.to_datetime(uccle.index, format='%Y-%m')
uccle['dindex'] = uccle.dateindex.dt.date.apply(lambda x: x.strftime('%Y-%m'))
l1 = uccle.dindex.tolist()
print('l1', l1)
...
print('common', len(common_dates12))
uccle = uccle[uccle.dindex.isin(common_dates12)]
predictors = predictors[predictors.dindex.isin(common_dates12)]

predictors = predictors.drop(['dateindex', 'dindex'], axis=1)
...
plotmlr_perkm(uccle.dateindex,uY,regression_output['fit_values'],'Total O3 Yearly Means','TotalOzone_yearly_mean_seasonal_smoothed')
...
plt.plot(uccle.index, uccle['TO'], color = '#1f77b4', label = 'Total ozone yearly means')
...
plt.savefig('/home/poyraden/Analysis/MLR_Uccle/Plots/Paper_Reviews/TO_yearly_trend_smoothed.pdf')
plt.savefig('/home/poyraden/Analysis/MLR_Uccle/Plots/Paper_Reviews/TO_yearly_trend_smoothed.png')
plt.savefig('/home/poyraden/Analysis/MLR_Uccle/Plots/Paper_Reviews/TO_yearly_trend_smoothed.eps')
