...
def load_independent_linear(pre_trend_end='1997-01-01', post_trend_start='2000-01-01', start_year = '1969-01-01', end_year = '2018-12-01'):
    ...
    start_year = pd.to_datetime('1992-11-01', format='%Y-%m-%d')
...
def load_enso(lag_months=0):
    ...
    data = pd.read_table('/home/poyraden/MLR_Uccle/Files/enso_debilt.dat', skiprows=1, sep='\s+',index_col=0,
                         engine='python', header=None)

    assert (data.index[0] == 1992)
    data = data.stack()
    data = data[data > -998]
...
def load_solar():
    ...
    page = sess.get(
        'https://omniweb.gsfc.nasa.gov/cgi/nx1.cgi?activity=retrieve&res=daily&spacecraft=omni2_daily&start_date=19921101&end_date={}&vars=50&scale=Linear&ymin=&ymax=&charsize=&symsize=0.5&symbol=0&imagex=640&imagey=480'.format(
...
predictors_debilt = linear_trends
predictors_debilt['enso'] = enso
predictors_debilt.loc['2018-12-01']['enso'] = predictors_debilt.loc['2018-11-01']['enso']
predictors_debilt['qboA'] = (QBO.pca - QBO.pca.mean())/QBO.pca.std()
predictors_debilt['qboB'] = (QBO.pcb - QBO.pcb.mean())/QBO.pcb.std()
predictors_debilt['solar'] = norsolar




predictors_debilt.to_csv('/home/poyraden/MLR_Uccle/Files/DeBilt_ilt.csv')
